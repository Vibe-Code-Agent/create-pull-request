 ...
 */
async function handleContentGenerationAndEditing(aiDescriptionService, spinner, ticketInfo, gitChanges, selectedTemplate, diffContent, options, repo, currentBranch) {
 ...
 */
async function handlePRCreation(options, repo, currentBranch, baseBranch, finalTitle, finalBody, finalSummary, jiraTicket, gitService, githubService, spinner) {
    if (options.dryRun) {
...
export async function createPullRequest(options) {
    ...
    try {
        ...
        const contentResult = await handleContentGenerationAndEditing(aiDescriptionService, spinner, ticketInfo, gitChanges, selectedTemplate, diffContent, options, repo, currentBranch);
        if (!contentResult) {
            return; // User cancelled
        }
        const { finalTitle, finalBody, finalSummary } = contentResult;
        // Handle PR creation or dry run
        await handlePRCreation(options, repo, currentBranch, baseBranch, finalTitle, finalBody, finalSummary, jiraTicket, gitService, githubService, spinner);
    }
    catch (error) {
